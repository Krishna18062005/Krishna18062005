<h1 align="center">🎮 Welcome to My Gaming Arena! 🎮</h1>
<h3 align="center">I'm Krishna | Code Warrior | Leveling Up Everyday</h3>

<p align="center">
  <!-- Stars -->
  <img src="https://img.shields.io/github/stars/Krishna18062005?label=Stars&style=for-the-badge&color=yellow" alt="GitHub Stars" />
  <!-- Visitor Count -->
  <img src="https://komarev.com/ghpvc/?username=Krishna18062005&label=Visitors&color=ff69b4&style=for-the-badge" alt="Profile Views" />
</p>

---

<table>
  <tr>
    <td valign="top" width="60%">
      <h3>🎯 <strong>Player Stats</strong></h3>
      <ul>
        <li>🔥 <strong>Current Level</strong>: ML Developer, Problem Solver, & Competitive Coder</li>
        <li>🛠 <strong>Weapons</strong>: Java, Python, TensorFlow, Keras, Git, Next.js, SQL</li>
        <li>🧩 <strong>Quest</strong>: Conquering LeetCode & Hackathon Challenges</li>
        <li>⚔️ <strong>Special Move</strong>: Building AI Solutions (e.g., Heart Disease Prediction using Federated Learning)</li>
      </ul>
    </td>
    <td valign="top" width="40%">
      <h3>📊 <strong>My Coding Profiles</strong></h3>
      <ul>
        <li><strong>SkillRack:</strong> 1760+ problems solved</li>
        <li><strong>LeetCode:</strong> 417+ problems solved</li>
        <li><strong>CodeChef:</strong> 403+ problems solved</li>
        <li><strong>Coding Ninjas:</strong> 832+ challenges completed</li>
      </ul>
    </td>
  </tr>
</table>

---

<h2 align="center">🏆 My Achievements & Battles</h2>

<ul>
  <li>🏅 <strong>Best Coding Engineer Award 2023</strong><br>
      Conquered the State Level Coding Competition at <strong>CADD Centre</strong>, Trichy.</li>
  <li>🏅 <strong>Daksh Hackathon 2025</strong><br>
      Secured a grant of INR 10,000 among top teams at <strong>Sastra Deemed University</strong>, Thanjavur.</li>
  <li>🏅 <strong>Discovering the Master Minds of Code Winner 2024</strong><br>
      Triumphed in the Intra-College Event at <strong>K. Ramakrishnan College of Engineering</strong>, Trichy.</li>
  <li>🏅 <strong>Sri Krishna College of Technology (Autonomous), Coimbatore</strong><br>
      - 2nd Prize in <em>Blind Coding</em><br>
      - 3rd Prize in <em>Code Relay</em><br>
      - 3rd Prize in <em>Neural Nector</em></li>
  <li>🏅 <strong>Care College of Engineering (Autonomous), Trichy</strong><br>
      Clinched First Prize in the <em>Debugging Competition</em>.</li>
</ul>

---

<h2 align="center">📚 Professional Journey</h2>

<ul>
  <li><strong>B.Tech in CSBS</strong><br>
      K. Ramakrishnan College of Engineering | CGPA: 8.16 | Expected Graduation: May 2026</li>
  <li><strong>Key Projects:</strong>
    <ul>
      <li><em>Heart Disease Prediction Using Federated Learning</em>: Achieved 92% accuracy while preserving data privacy.</li>
      <li><em>FactoryFlowAI</em>: Optimized manufacturing workflows with real-time monitoring and AI-driven task management.</li>
      <li><em>Personal Portfolio & Achievements Website</em>: Developed with Next.js and Tailwind CSS.</li>
    </ul>
  </li>
</ul>

---

<h2 align="center">🌟 Leadership Experience</h2>

<ul>
  <li><strong>Team Leader, Innovate and Inspire</strong> (K. Ramakrishnan College of Engineering)<br>
      - Led a team of 10+ members to organize 100+ technical and non-technical events.<br>
      - Executed ten seasons of national-level symposiums with 10000+ participants.<br>
      - Collaborated with faculty and peers to ensure high engagement and smooth event coordination.</li>
  <li><strong>Vice President, The Institution of Engineers (India) Club</strong><br>
      - Organized and led 10+ offline events in collaboration with The Institution of Engineers (India).<br>
      - Coordinated with students and faculty to drive technical excellence and community engagement.<br>
      - Fostered an environment of innovation through structured technical events.</li>
</ul>


---

<h2 align="center">🤝 Let's Connect</h2>


